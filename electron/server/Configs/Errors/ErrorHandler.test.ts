...
type TestError = Error & { code?: ErrorCodeType; contexts?: string[] }
...
describe('ErrorHandler', () => {
  describe('throwError', () => {
    ...
    it('should throw an error with default code if not provided', () => {
      expect(() => {
        throwError('Test error', undefined as unknown as ErrorCodeType)
      }).toThrow('Test error')
    })
  })
  ...
  describe('processError', () => {
    it('should process Error instances correctly', () => {
      const error = new Error('Test error') as TestError;
  ...
  describe('middlewareError', () => {
    it('should return an Error object with the specified code', () => {
      ...
      expect((result as TestError).code).toBe(ErrorCode.ACCESS_DENIED)
      expect((result as TestError).contexts).toEqual([])
    })
  })
  ...
  describe('handleIpcError', () => {
    ...
    beforeEach(() => {
      loggerErrorSpy = vi.spyOn(logger, 'error').mockImplementation((() => {}) as unknown as typeof logger.error)
    })
    ...
    it('should normalize error and return an IPC compatible error object', () => {
      const error = new Error('Database connection failed') as TestError;
      error.code = ErrorCode.INTERNAL_ERROR;
  ...
  describe('wrapIpcHandler', () => {
    ...
    it('should return ok: false and normalized error when handler fails', async () => {
      const error = new Error('Action failed') as TestError;
      error.code = ErrorCode.VALIDATION_ERROR;
