...
import { appointmentService } from '../../Shared/dependencies.js';

export default {
  createAppointment: (data: unknown) => {
    const validData = NodeValidator.validateBody(data, sch.createAppointmentSchema);
  ...
  getAppointmentsByRange: (data: unknown) => {
    const validData = NodeValidator.validateBody(data, sch.getByRangeSchema);
  ...
  updateAppointmentStatus: (data: unknown) => {
    const validData = NodeValidator.validateBody(data, sch.updateStatusSchema);
  ...
  deleteAppointment: (data: unknown) => {
    const validData = NodeValidator.validateBody(data, sch.deleteAppointmentSchema);
    const appointmentId = NodeValidator.paramId(validData, 'appointmentId', UuidHandler.regexUuid);
